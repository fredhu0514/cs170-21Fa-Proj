...
def solve(tasks):
    ...
    G.initialize_paths()
    return Bellman_Ford(G, len(tasks) * 1440 + 1)
...
class TaskGraph():
        ...
        for v in self.edges:
            ...
            for i in range(self.task_num):
                if i != v[0] and (i, t_after) in self.vertex:
                    self.egde.add((v, (i, t_after)))
            if v[1] == 0:
                self.edge.add(self.start, v)
    ...
    def initialize_paths(self):
        for v in self.vertex:
            self.distances[v] = [list() for i in range(self.task_num)]
    ...
    def update(self, edge):
        ...
        for i in range(self.task_num):
            if i != task_index2:
                ...
                print(self.distances[v2][i])
                print(task_index2)
                print(self.paths[v1][i])
                ...
                if new_path_length < self.mins[v2][i] and task_index2 not in self.paths[v1][i]:
                    new_path = self.paths[v1][i].copy()
                    new_path.append(v2[0])
                    self.mins[v2][i] = new_path_length
                    self.min_paths[v2][i] = new_path
            else:
                if new_path_length < self.mins[v2][i] and task_index2 not in self.paths[v1][i]:
                    new_path = self.paths[v1][i].copy()
                    new_path.append(v2[0])
...
def Bellman_Ford(G, num_iteration):
    ...
    task_ids = []
    for v in path:
        task_ids.append(G.tasks[v[0]].get_task_id())
