...
if __name__ == '__main__':
    logging.basicConfig(filename="GreedySolver3.log", level=logging.INFO)
    logging.info('New Log at ' + datetime.now().strftime('%m/%d/%Y %H:%M:%S'))
    for max_n in range(2, 51):
        print('max n = ' + str(max_n) + '...')
        logging.info('max n = ' + str(max_n) + ':')
        ...
        for x in benefits.keys():
            for input_path in os.listdir('inputs/'+x+'/'):
                if input_path[-2:] == 'in':
                    ...
                    output, benefit = solve_iter(tasks, max_n)
