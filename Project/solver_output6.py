...
def all_or_nothing(task, CurrTime, TotalTime):
    if CurrTime + task.get_duration() > task.get_deadline():
        return 0
    else:
...
def SortTasks(tasks, CurrTime: int, TotalTime: int, By: str = 'Profit'):
    ...
    """
    if By == 'Profit':
        tasks.sort(key = lambda task: all_or_nothing(task, CurrTime, TotalTime)/task.get_duration(), reverse = True)
    elif By == 'Deadline':
...
if __name__ == '__main__':
    logging.basicConfig(filename="GreedySolver6py.log", level=logging.INFO)
    logging.info('New Log at ' + datetime.now().strftime('%m/%d/%Y %H:%M:%S'))
    benefits = {'small':[], 'medium':[], 'large':[]}
    if not os.path.isdir('outputs6/'):
        os.mkdir('outputs6/')
    for x in benefits.keys():
        if not os.path.isdir('outputs6/'+x+'/'):
            os.mkdir('outputs6/'+x+'/')
        for input_path in os.listdir('inputs/'+x+'/'):
            if input_path[-2:] == 'in':
                ...
                write_output_file('outputs6/'+x+'/'+input_path[:-2]+'out', bestOutput)
