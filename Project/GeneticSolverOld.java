...
public class GeneticSolverOld {

    Random generator;
    public GeneticSolverOld(long seed){
    ...
    private class FitnessSort implements Comparator<List<Integer>> {
        // Used for sorting in descending order of
        // total profit of a task sequence
    ...
    public List<List<Integer>> genetic_cycle(List<Task> tasks, List<List<Integer>> parents, double crossover_prob, double point_mutation_prob, double second_mutation_prob, double deletion_prob, int capacity) {
        ...
        for (int l = 0; l < num_crossovers; l++) {
            ...
            if (new HashSet<>(offspring_chromo1).size() == offspring_chromo1.size()) { // make sure no repeitions in tasks
            ...
            if (new HashSet<>(offspring_chromo2).size() == offspring_chromo2.size()) {
    ...
    public List<Integer> solve(List<Task> tasks, List<List<Integer>> initial_outputs, double crossover_prob, double point_mutation_prob, double second_mutation_prob, double deletion_prob, int capacity, int threshold) {
        ...
        while (stuck_cycles < threshold) {
            List<List<Integer>> offsprings = genetic_cycle(tasks, parents, crossover_prob, point_mutation_prob, second_mutation_prob, deletion_prob, capacity);
    ...
    public static List<Integer> solveInputs(String input_path, List<String> UnImprovables) throws Exception{
        ...
        List<Integer> output0 = parse.read_output_file("Project/outputs/" + input_path.substring(0, input_path.length()-2)+"out");
        ...
        List<List<Integer>> initial_outputs = new ArrayList<>(List.of(output0, output1, output2, output3));
        List<Integer> output = new ArrayList<>();
        List<List<Integer>> new_outputs = new ArrayList<>();
        for (int i = 1; i <= 30; i++) {
            output = new GeneticSolverOld(i).solve(tasks, initial_outputs, 0.5, 0.10, 0.05, 0.02, 30, 50);
            new_outputs.add(output);
        }
        GeneticSolverOld GrandSolver = new GeneticSolverOld(0);
        Collections.sort(new_outputs, GrandSolver.new FitnessSort(tasks));
        new_outputs = new_outputs.subList(20, 30);
        if (new_outputs.get(0).equals(new_outputs.get(9))) {
            String output_path = "OutputsGeneticOld/" + input_path.substring(0, input_path.length()-2)+"out";
            parse.write_output_file(output_path, output);
            UnImprovables.add(input_path); //this file actually has no use lol
            return output;
        }
        for (int c = 1; c <= 30; c++) {
            for (int i = 1; i <= 30; i++) {
                output = new GeneticSolverOld(i).solve(tasks, new_outputs, 0.5, 0.10, 0.05, 0.02, 30, 50);
                new_outputs.add(output);
    ...
    public static void main(String[] args) throws Exception{
        Logger logger = Logger.getLogger("GeneticSolverJavaOld");
        FileHandler fh;
        try {
            // This block configure the logger with handler and formatter
            fh = new FileHandler("GeneticSolverJavaOld.log");
            logger.addHandler(fh);
            SimpleFormatter formatter = new SimpleFormatter();
            fh.setFormatter(formatter);

            File outputFolder = new File("OutputsGeneticOld/");
            ...
            for (String key:keys) {
                File currFolder = new File("OutputsGeneticOld/" + key + "/");
