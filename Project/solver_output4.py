...
def solve(tasks):
    ...
    """
    tasks = SortTasks(tasks) # sort tasks by deadline
    CurrSeq = []
    CurrTime = 0
...
def SortTasks(tasks):
    ...
    -   tasks: list[Task], list of igloos to polish
    Output:
    -   tasks: list[Task], list of igloos to polish sorted by deadline
    """
...
if __name__ == '__main__':
    ...
    for x in benefits.keys():
        if not os.path.isdir('outputs4/'+x+'/'):
            os.mkdir('outputs4/'+x+'/')
        for input_path in os.listdir('inputs/'+x+'/'):
            if input_path[-2:] == 'in':
                ...
                output = solve(tasks)
                write_output_file('outputs4/'+x+'/'+input_path[:-2]+'out', output)
                tasks = read_input_file(path)
